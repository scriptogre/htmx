describe('hx-confirm attribute', function () {
  ...
  it('can disable confirmation by setting ctx.confirm to null', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-confirm="Should not see">Click</button>',
    )
  ...
  it('works with forms', async function () {
    ...
    let form = createProcessedHTML(
      '<form hx-post="/submit" hx-confirm="Submit form?"><button>Submit</button></form>',
    )
    form.requestSubmit()
    await forRequest()
  ...
  it('cleans up indicators when confirmation cancelled', async function () {
    ...
    mockResponse('GET', '/test', 'Success')
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-confirm="Cancel?" hx-indicator="#ind">Click</button>',
    )
  ...
  it('re-enables disabled elements when confirmation cancelled', async function () {
    ...
    mockResponse('GET', '/test', 'Success')
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-confirm="Cancel?" hx-disable>Click</button>',
    )
