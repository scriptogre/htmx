describe('hx-boost attribute', function () {
  ...
  it('handles basic anchor properly', async function () {
    ...
    createProcessedHTML(
      '<div hx-target:inherited="this" hx-swap:inherited="outerHTML" hx-boost:inherited="true"><a id="a1" href="/test">Foo</a></div>',
    )
    find('#a1').click()
    await forRequest()
  ...
  it('handles basic form post properly', async function () {
    ...
    createProcessedHTML(
      '<div hx-target:inherited="this" hx-swap:inherited="outerHTML" hx-boost:inherited="true"><form id="f1" action="/test" method="post"><button id="b1">Submit</button></form></div>',
    )
    find('#b1').click()
    await forRequest()
    playground().innerHTML.should.equal('Boosted')
  ...
  it('handles basic form post with button formaction properly', async function () {
    mockResponse('POST', '/test', 'Boosted')
    createProcessedHTML(
      '<div hx-target:inherited="this" hx-swap:inherited="outerHTML" hx-boost:inherited="true"><form id="f1" action="/bad" method="post"><button id="b1" formaction="/test">Submit</button></form></div>',
    )
    find('#b1').click()
    await forRequest()
    playground().innerHTML.should.equal('Boosted')
  ...
  it('handles basic form post with button formmethod properly', async function () {
    mockResponse('POST', '/test', 'Boosted')
    createProcessedHTML(
      '<div hx-target:inherited="this" hx-swap:inherited="outerHTML" hx-boost:inherited="true"><form id="f1" action="/test" method="get"><button id="b1" formmethod="post">Submit</button></form></div>',
    )
    find('#b1').click()
    await forRequest()
    playground().innerHTML.should.equal('Boosted')
  ...
  it('handles basic form post with button formmethod & formaction properly', async function () {
    mockResponse('POST', '/test', 'Boosted')
    createProcessedHTML(
      '<div hx-target:inherited="this" hx-swap:inherited="outerHTML" hx-boost:inherited="true"><form id="f1" action="/bad" method="get"><button id="b1" formmethod="post" formaction="/test">Submit</button></form></div>',
    )
    find('#b1').click()
    await forRequest()
    playground().innerHTML.should.equal('Boosted')
  ...
  it('handles basic form post properly w/ explicit action', async function () {
    mockResponse('POST', '/test', 'Boosted')
    createProcessedHTML(
      '<div hx-target:inherited="this" hx-swap:inherited="outerHTML"><form id="f1" action="/test" method="post"  hx-boost="true"></form></div>',
    )
  ...
  it('does not boost forms with method="dialog"', async function () {
    mockResponse('GET', '/test', 'Boosted')
    createProcessedHTML(
      '<div hx-boost:inherited="true"><form id="f1" action="/test" method="dialog"><button id="b1">close</button></form></div>',
    )
    find('#b1').click()
  ...
  it('does not boost forms with buttons with formmethod="dialog"', async function () {
    mockResponse('GET', '/test', 'Boosted')
    createProcessedHTML(
      '<div hx-boost:inherited="true"><form id="f1" action="/test" method="get"><button formmethod="dialog" id="b1">close</button></form></div>',
    )
    find('#b1').click()
    fetchMock.calls.length.should.equal(0)
  })
