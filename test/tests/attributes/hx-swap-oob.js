describe('hx-swap-oob', function () {
  ...
  it('swaps oob element by id with default outerHTML', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="oob1" hx-swap-oob="true">OOB Content</div>',
    )
  ...
  it('swaps oob element with innerHTML', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="oob2" hx-swap-oob="innerHTML">New Inner</div>',
    )
  ...
  it('swaps oob element with custom selector', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="x" hx-swap-oob="outerHTML:#target">OOB Target</div>',
    )
  ...
  it('swaps multiple oob elements', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="a" hx-swap-oob="true">A</div><div id="b" hx-swap-oob="true">B</div>',
    )
    createProcessedHTML(
      '<div hx-get="/test">Click</div><div id="a">Old A</div><div id="b">Old B</div>',
    )
    find('[hx-get]').click()
    await forRequest()
  ...
  it('swaps oob with target: modifier', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="x" hx-swap-oob="innerHTML target:#custom">Target Content</div>',
    )
  ...
  it('swaps oob with target: modifier and multi-word selector', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="x" hx-swap-oob="innerHTML target:&quot;.foo .bar&quot;">Multi Selector</div>',
    )
    createProcessedHTML(
      '<div hx-get="/test">Click</div><div class="foo"><div class="bar">Original</div></div>',
    )
    find('[hx-get]').click()
    await forRequest()
  ...
  it('swaps oob with target: modifier and other modifiers (non-blocking)', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="x" hx-swap-oob="innerHTML target:#tgt swap:100ms">With Delay</div>',
    )
  ...
  it('swaps oob with legacy colon format', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div>Main</div><div id="x" hx-swap-oob="innerHTML:#legacy">Legacy Format</div>',
    )
