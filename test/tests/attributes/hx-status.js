describe('hx-status attribute tests', function () {
  ...
  it('applies swap override for wildcard pattern', async function () {
    ...
    createProcessedHTML(
      '<div id="target">Original</div><button hx-get="/test" hx-target="#target" hx-status:5xx="swap:innerHTML">Click</button>',
    )
    let button = find('button')
    button.click()
    await forRequest()
  ...
  it('can change target based on status', async function () {
    createProcessedHTML(
      '<div id="success"></div><div id="error"></div><button hx-get="/test" hx-target="#success" hx-status:404="target:#error">Click</button>',
    )
  ...
  it('can select different content based on status', async function () {
    mockResponse(
      'GET',
      '/test',
      '<div id="success-msg">Success!</div><div id="error-msg">Failed!</div>',
      { status: 422 },
    )
    createProcessedHTML(
      '<div id="target"></div><button hx-get="/test" hx-target="#target" hx-status:422="select:#error-msg">Click</button>',
    )
    let button = find('button')
    button.click()
    await forRequest()
  ...
  it('can set multiple properties with hx-status', async function () {
    createProcessedHTML(
      '<div id="main"></div><div id="errors"></div><button hx-get="/test" hx-target="#main" hx-status:422="swap:innerHTML target:#errors select:#validation-errors">Click</button>',
    )
  ...
  it('prefers exact match over wildcard', async function () {
    ...
    createProcessedHTML(
      '<div id="target"></div><button hx-get="/test" hx-target="#target" hx-status:404="select:#specific" hx-status:4xx="select:#generic">Click</button>',
    )
  ...
  it('can prevent history update on error', async function () {
    ...
    createProcessedHTML(
      '<div id="target"></div><button hx-get="/test" hx-target="#target" hx-push-url="true" hx-status:5xx="push:false">Click</button>',
    )
  ...
  it('works with 2-digit wildcard pattern', async function () {
    ...
    createProcessedHTML(
      '<div id="target"></div><button hx-get="/test" hx-target="#target" hx-status:50x="swap:innerHTML">Click</button>',
    )
    let button = find('button')
    button.click()
    await forRequest()
  ...
  it('does not apply when status does not match', async function () {
    ...
    createProcessedHTML(
      '<div id="target">Original</div><button hx-get="/test" hx-target="#target" hx-status:404="swap:none">Click</button>',
    )
    let button = find('button')
    button.click()
    await forRequest()
  ...
  it('can set swap to none on error', async function () {
    ...
    createProcessedHTML(
      '<div id="target">Original</div><button hx-get="/test" hx-target="#target" hx-status:500="swap:none">Click</button>',
    )
    let button = find('button')
    button.click()
    await forRequest()
