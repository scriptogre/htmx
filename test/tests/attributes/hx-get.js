describe('hx-get attribute', function () {
  ...
  it.skip('GET does not include surrounding data by default', async function () {
    ...
    mockResponse('GET', '/test', 'Clicked!')
    createProcessedHTML(
      '<form><input name="i1" value="value"/><p id="p1"><button id="b1" hx-get="/test">Click Me!</button></p></form>',
    )
    find('#b1').click()
    await forRequest()
  ...
  it('GET on form includes its own data by default', async function () {
    ...
    let form = createProcessedHTML(
      '<form hx-get="/test" hx-swap="outerHTML"><input name="i1" value="value"/><button id="b1">Click Me!</button></form>',
    )
    form.requestSubmit()
    await forRequest()
  ...
  it('GET on form with existing parameters works properly', async function () {
    mockResponse('GET', '/test', 'Clicked!')
    let form = createProcessedHTML(
      '<form hx-get="/test?foo=bar" hx-swap="outerHTML"><input name="i1" value="value"/><button id="b1">Click Me!</button></form>',
    )
    form.requestSubmit()
    await forRequest()
    playground().innerHTML.should.equal('Clicked!')
  ...
  it('GET on form with existing parameters works properly', async function () {
    mockResponse('GET', '/test', 'Clicked!')
    let form = createProcessedHTML(
      '<form hx-get="/test?foo=bar#foo" hx-swap="outerHTML"><input name="i1" value="value"/><button id="b1">Click Me!</button></form>',
    )
    form.requestSubmit()
    await forRequest()
    playground().innerHTML.should.equal('Clicked!')
  ...
  it('GET on form with anchor works properly and scrolls to anchor id', async function () {
    ...
    let form = createProcessedHTML(
      '<form hx-trigger="click" hx-get="/test?foo=bar#foo" hx-swap="outerHTML"><input name="i1" value="value"/><button id="b1">Click Me!</button></form>',
    )
    form.click()
    await forRequest()
