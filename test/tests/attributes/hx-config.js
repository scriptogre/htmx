describe('hx-config attribute', function () {
  ...
  it('overrides action with JSON config', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-trigger="click" hx-config=\'{"action": "/override"}\'>Click</button>',
    )
    btn.click()
    await forRequest()
    playground().innerText.should.equal('Overridden!')
    assert.isTrue(lastFetch().url.startsWith('/override'))
    assert.equal(ctx.request.shouldActivate, '/override')
  })
  ...
  it('overrides validate via config', async function () {
    ...
    let form = createProcessedHTML(
      '<form hx-post="/test" hx-config=\'{"validate": false}\'><input required name="test" value="filled"><button>Submit</button></form>',
    )
    form.requestSubmit()
    await forRequest()
  ...
  it('merges into options with + prefix', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"cache": "no-cache"}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('can set custom property on config', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"customProperty": "myValue"}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('can set multiple custom properties', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"prop1": "value1", "prop2": "value2", "prop3": 123}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('can override method via config', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"method": "PUT"}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('supports boolean values in config', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"myFlag": true, "otherFlag": false}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('supports numeric values in config', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"timeout": 5000, "retries": 3}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('supports null values in config', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-select="#foo" hx-config=\'{"select": null}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('merges non-object values with + prefix', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-trigger="click" hx-config=\'{"+action": "/newpath"}\'>Click</button>',
    )
    btn.click()
    await forRequest()
    assert.isTrue(ctx.request.shouldActivate.startsWith('/newpath'))
  })
  ...
  it('handles complex nested JSON structures', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"custom": {"nested": {"deep": "value"}}}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('merges arrays by replacement not concatenation', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"myArray": [1, 2, 3]}\'>Click</button>',
    )
    btn.click()
    await forRequest()
  ...
  it('can override validation setting via config', async function () {
    ...
    let form = createProcessedHTML(
      '<form hx-post="/submit" hx-validate="true" hx-config=\'{"validate": false}\'><input required name="test" value="test"><button>Submit</button></form>',
    )
    form.requestSubmit()
    await forRequest()
    assert.isFalse(ctx.request.validate)
  ...
  it('multiple elements with different configs work independently', async function () {
    ...
    createProcessedHTML(
      '<button id="b1" hx-get="/test" hx-trigger="click" hx-swap="innerHTML" hx-config=\'{"action": "/path1"}\'>B1</button><button id="b2" hx-get="/test" hx-trigger="click" hx-swap="innerHTML" hx-config=\'{"action": "/path2"}\'>B2</button>',
    )
  ...
  it('config can be inherited with :inherited suffix', async function () {
    ...
    createProcessedHTML(
      '<div hx-config:inherited=\'{"action": "/inherited"}\'><button hx-get="/original" hx-trigger="click">Click</button></div>',
    )
    find('button').click()
    await forRequest()
    playground().innerText.should.equal('Inherited config')
    assert.isTrue(lastFetch().url.startsWith('/inherited'))
    assert.equal(ctx.request.shouldActivate, '/inherited')
  })
  ...
  it('child config takes precedence over inherited config', async function () {
    ...
    createProcessedHTML(
      '<div hx-config:inherited=\'{"action": "/parent"}\'><button hx-get="/original" hx-trigger="click" hx-config=\'{"action": "/child"}\'>Click</button></div>',
    )
    find('button').click()
    await forRequest()
    assert.isTrue(lastFetch().url.startsWith('/child'))
    assert.equal(ctx.request.shouldActivate, '/child')
  })
  ...
  it('merges headers with + prefix', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-config=\'{"+headers": {"X-Custom": "value"}}\'>Click</button>',
    )
    btn.click()
    await forRequest()
