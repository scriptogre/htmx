...
describe('scroll restoration on history traversal', function() {
    ...
    it('boosted back restores content, then the saved scroll position', async function() {
        // page A is tall enough to scroll; page B is short, so a scroll restore
        // against the outgoing DOM could only clamp
        playground().innerHTML = '<main hx-history-elt><div style="height:3000px">page A</div></main>';
        htmx.process(playground());
        htmx.__replaceUrlInHistory('/scroll-page-a');
    ...
    it('back to an entry created by an anchor jump still restores content', async function() {
        ...
        htmx.process(playground());
        htmx.__replaceUrlInHistory('/scroll-ref');
    ...
    it('restores horizontal scroll as well', async function() {
        ...
        htmx.process(playground());
        htmx.__replaceUrlInHistory('/scroll-wide');
    ...
    it('restores to top when no position is saved', async function() {
        // simulates an entry from before a full page load: htmx state, no in-memory position
        playground().innerHTML = '<main hx-history-elt><div style="height:3000px">current page</div></main>';
        htmx.process(playground());
        history.replaceState({htmx: true}, '', '/unknown-scroll');
        htmx.__pushUrlIntoHistory('/known-page');
        window.scrollTo(0, 400);

        mockResponse('GET', '/unknown-scroll',
            '<html><body><main hx-history-elt><p>unknown restored</p></main></body></html>');

        history.back();
        await forRequest();
        await htmx.timeout(50);

        playground().textContent.should.include('unknown restored');
    ...
    it('hash-only traversal neither refetches nor scrolls', async function() {
        playground().innerHTML = '<main hx-history-elt><p>hash page</p></main>';
        htmx.process(playground());
        htmx.__replaceUrlInHistory('/hash-page');
