describe('Server-Sent Events', function () {
  ...
  it('continuous stream reconnects with exponential backoff', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/reconnect" hx-config="sse.reconnect:true sse.reconnectDelay:50ms sse.reconnectMaxAttempts:3 sse.reconnectJitter:0" hx-swap="innerHTML">Connect</button>',
    )
    ...
    assert.equal(
      reconnectDelays[0],
      50,
      'First reconnect should use initial delay (reconnectDelay)',
    )
  ...
  it('Last-Event-ID header sent on reconnect', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/with-id" hx-config="sse.reconnect:true sse.reconnectDelay:50ms sse.reconnectMaxAttempts:2" hx-swap="innerHTML">Connect</button>',
    )
    ...
    assert.equal(
      lastCall.request.headers['Last-Event-ID'],
      'msg-123',
      'Last-Event-ID header should be set',
    )
  ...
  it('events fire correctly and can cancel operations', async function () {
    ...
    document.addEventListener('htmx:before:sse:stream', () => {
      beforeStreamFired = true
    })
    document.addEventListener('htmx:before:sse:message', () => {
      beforeMessageFired = true
    })
    document.addEventListener('htmx:after:sse:message', () => {
  ...
  it('element removal stops streaming', async function () {
    ...
    createProcessedHTML(
      '<div id="container"><button hx-get="/removable" hx-config="sse.reconnect:true sse.reconnectDelay:50ms" hx-swap="innerHTML">Connect</button></div>',
    )
  ...
  it('SSE message parsing with id field', async function () {
    ...
    createProcessedHTML(
      '<div id="target"></div><button hx-get="/parse" hx-target="#target">Go</button>',
    )
  ...
  it('reconnectMaxAttempts configuration works', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/max-attempts" hx-config="sse.reconnect:true sse.reconnectDelay:20ms sse.reconnectMaxAttempts:2" hx-swap="innerHTML">Connect</button>',
    )
  ...
  it('reconnectMaxDelay configuration caps backoff', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/max-delay" hx-config="sse.reconnect:true sse.reconnectDelay:20ms sse.reconnectMaxDelay:60ms sse.reconnectJitter:0" hx-swap="innerHTML">Connect</button>',
    )
  ...
  it('reconnectJitter randomizes reconnect delays', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/jitter-test" hx-config="sse.reconnect:true, sse.reconnectDelay:100ms, sse.reconnectJitter:0.5, sse.reconnectMaxAttempts: 5" hx-swap="innerHTML">Connect</button>',
    )
    ...
    let allIdentical = reconnectDelays.every(
      (delay, i, arr) => i === 0 || Math.abs(delay - arr[i - 1]) < 1,
    )
  ...
  it('reconnectJitter of 0 disables jitter', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/no-jitter" hx-config="sse.reconnect:true, sse.reconnectDelay:100ms, sse.reconnectJitter:0, sse.reconnectMaxAttempts:3" hx-swap="innerHTML">Connect</button>',
    )
  ...
  it('custom events work with hx-on attribute', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/hx-on-test" hx-swap="innerHTML" hx-on:progress="this.setAttribute(\'data-progress\', event.detail.data)">Connect</button>',
    )
    ...
    assert.equal(
      find('button').getAttribute('data-progress'),
      '50%',
      'hx-on should handle custom event',
    )
    ...
    assert.equal(
      find('button').getAttribute('data-progress'),
      '100%',
      'hx-on should handle multiple custom events',
    )
