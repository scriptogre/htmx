describe('Strip Modifier', function () {
  ...
  it('OOB innerHTML with strip:false keeps wrapper', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><article hx-swap-oob="innerHTML target:#oob strip:false"><h1>Title</h1><p>Text</p></article>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><div id="oob">Original</div>',
    )
  ...
  it('OOB beforeend with default strip extracts children', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><div hx-swap-oob="beforeend:#list"><li>Item A</li><li>Item B</li></div>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><ul id="list"><li>Existing</li></ul>',
    )
    find('#btn').click()
    await forRequest()
  ...
  it('OOB beforeend with strip:false keeps wrapper element', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><li hx-swap-oob="beforeend target:#list strip:false"><strong>Item</strong></li>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><ul id="list"><li>Existing</li></ul>',
    )
  ...
  it('OOB outerHTML with default (no strip) keeps wrapper', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><wrapper hx-swap-oob="outerHTML:#target"><section>A</section><section>B</section></wrapper>',
    )
    createProcessedHTML(
  ...
  it('OOB outerHTML with strip:true replaces with multiple children', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><wrapper hx-swap-oob="outerHTML strip:true target:#target"><section>A</section><section>B</section><aside>C</aside></wrapper>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><div id="target">Old</div>',
    )
  ...
  it('OOB with SVG and default strip preserves namespace', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><svg hx-swap-oob="beforeend:#canvas"><circle cx="50" cy="50" r="20"></circle><circle cx="100" cy="100" r="30"><circle></svg>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><svg id="canvas" xmlns="http://www.w3.org/2000/svg"></svg>',
    )
    find('#btn').click()
    await forRequest()
  ...
  it('OOB with SVG and strip:false creates nested SVG', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><svg hx-swap-oob="beforeend target:#canvas strip:false" xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="20"></circle></svg>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><svg id="canvas" xmlns="http://www.w3.org/2000/svg"></svg>',
    )
  ...
  it('Partial with SVG and strip:true extracts circles', async function () {
    mockResponse(
      'GET',
      '/api',
      '<div id="main">Main</div><hx-partial hx-target="#canvas" hx-swap="innerHTML strip:true"><svg xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="20"/><circle cx="100" cy="100" r="30"/></svg></hx-partial>',
    )
    createProcessedHTML(
      '<button id="btn" hx-get="/api" hx-target="#main">Get</button><svg id="canvas" xmlns="http://www.w3.org/2000/svg"></svg>',
    )
