describe('Basic Functionality', () => {
  ...
  it('Button click triggers fetch and swaps content', async () => {
    ...
    // Create test button
    createProcessedHTML(
      '<button id="test-btn" hx-action="/demo" hx-target="#target">Click</button><div id="target">Original</div>',
    )
    // Click the button
    find('#test-btn').click()
    await forRequest()
  ...
  it('validation errors prevent submission of a form', async function () {
    ...
    createProcessedHTML(
      '<form><input id="i1" required/><button id="b1" hx-post="/demo" hx-validate="true">Demo</button></form>',
    )
  ...
  it('validation errors do not prevent submission of an element within a form marked as hx-validate=false', async function () {
    ...
    // Create test button
    createProcessedHTML(
      '<form><input id="i1" required/><button id="b1" hx-post="/demo" hx-validate="false">Demo</button></form>',
    )
