describe('parseTriggerSpecs unit tests', function () {
  it('parses single event', function () {
    let specs = htmx.parseTriggerSpecs('click')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
  })

  it('parses multiple events with comma', function () {
    let specs = htmx.parseTriggerSpecs('click, change')
  ...
  it('parses event with modifier value', function () {
    let specs = htmx.parseTriggerSpecs('click delay:100ms')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
  ...
  it('parses event with boolean modifier', function () {
    let specs = htmx.parseTriggerSpecs('click once')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
  ...
  it('parses event with multiple modifiers', function () {
    let specs = htmx.parseTriggerSpecs('click delay:100ms throttle:200ms once')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
    assert.equal(specs[0].delay, '100ms')
  ...
  it('parses event with from modifier', function () {
    let specs = htmx.parseTriggerSpecs('click from:#other')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
  ...
  it('parses event with target modifier', function () {
    let specs = htmx.parseTriggerSpecs('click target:.item')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
  ...
  it('parses event with consume modifier', function () {
    let specs = htmx.parseTriggerSpecs('click consume')
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click')
  ...
  it('parses event with changed modifier', function () {
    let specs = htmx.parseTriggerSpecs('change changed')
    assert.equal(specs.length, 1)
  ...
  it('parses every trigger', function () {
    let specs = htmx.parseTriggerSpecs('every 1s')
    assert.equal(specs.length, 1)
  ...
  it('handles empty string', function () {
    let specs = htmx.parseTriggerSpecs('')
  ...
  it('parses complex trigger spec', function () {
    let specs = htmx.parseTriggerSpecs(
      'click[ctrlKey] delay:100ms throttle:200ms from:body target:.item once',
    )
    assert.equal(specs.length, 1)
    assert.equal(specs[0].name, 'click[ctrlKey]')
    assert.equal(specs[0].delay, '100ms')
