describe('swap() unit tests', function() {
    ...
    // Tests below require htmx.__parseSwapSpec (monolithic architecture)
    const _skip = typeof htmx.__parseSwapSpec !== 'function' ? it.skip : it

    _skip('prepend normalizes to afterbegin', function () {
        assert.equal(htmx.__parseSwapSpec('prepend').style, 'afterbegin')
    })

    _skip('append normalizes to beforeend', function () {
        assert.equal(htmx.__parseSwapSpec('append').style, 'beforeend')
    })

    _skip('before normalizes to beforebegin', function () {
        assert.equal(htmx.__parseSwapSpec('before').style, 'beforebegin')
    })

    _skip('after normalizes to afterend', function () {
        assert.equal(htmx.__parseSwapSpec('after').style, 'afterend')
    })

    _skip('new terminology works with modifiers', function () {
    ...
    it.skip('triggers view transition events with transition:true', async function () {
    ...
    it.skip('restores focus after innerHTML swap when element has same id', async function () {
    ...
    it.skip('restores focus after outerHTML swap when element has same id', async function () {
    ...
    it.skip('restores focus to textarea after innerHTML swap', async function () {
