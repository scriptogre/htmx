describe('extractFilter unit tests', function () {
  it('returns event name and null when no filter', function () {
    let [event, filter] = htmx.extractFilter('click')
    assert.equal(event, 'click')
    assert.equal(filter, null)
  })

  it('extracts filter from brackets', function () {
    let [event, filter] = htmx.extractFilter('click[ctrlKey]')
    assert.equal(event, 'click')
  ...
  it('handles empty string', function () {
    let [event, filter] = htmx.extractFilter('')
    assert.equal(event, '')
    assert.equal(filter, null)
  })
})
