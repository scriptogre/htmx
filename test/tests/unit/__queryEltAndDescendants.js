describe('selectAll unit tests', function () {
  it('returns descendants matching selector', function () {
    ...
    let results = htmx.selectAll(div, '.foo')
  ...
  it('handles multiple matching descendants', function () {
    let div = createProcessedHTML(
      '<div><span class="foo"></span><span class="foo"></span><span class="foo"></span></div>',
    )
    let results = htmx.selectAll(div, '.foo')
  ...
  it('places element first when it matches', function () {
    let div = createProcessedHTML(
      '<div class="foo"><span class="foo"></span><p class="foo"></p></div>',
    )
    let results = htmx.selectAll(div, '.foo')
    assert.equal(results.length, 3)
    assert.equal(results[0], div)
