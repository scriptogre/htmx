describe('makeFragment unit tests', function () {
  it('creates fragment from simple HTML', function () {
    let { fragment } = htmx.makeFragment('<div>Test</div>')
  ...
  it('creates fragment from multiple elements', function () {
    let { fragment } = htmx.makeFragment('<div>A</div><span>B</span>')
  ...
  it('handles html tag', function () {
    let { fragment } = htmx.makeFragment('<html><body><div>Test</div></body></html>')
    fragment.children[0].tagName.should.equal('DIV')
  })

  it('handles body tag', function () {
    let { fragment } = htmx.makeFragment('<body><div>Test</div></body>')
    fragment.children[0].tagName.should.equal('DIV')
  })
  ...
  it('strips head content', function () {
    let { fragment } = htmx.makeFragment('<head><title>Title</title></head><div>Body</div>')
    assert.isNull(fragment.querySelector('head'))
    fragment.children[0].tagName.should.equal('DIV')
  })

  it('handles table row content', function () {
    let { fragment } = htmx.makeFragment('<tr><td>Cell 1</td><td>Cell 2</td></tr>')
  ...
  it('handles multiple table rows', function () {
    let { fragment } = htmx.makeFragment('<tr><td>A</td></tr><tr><td>B</td></tr>')
    fragment.children.length.should.equal(2)
    fragment.children[0].tagName.should.equal('TR')
  ...
  it('handles table cell content', function () {
    let { fragment } = htmx.makeFragment('<td>Cell Content</td>')
  ...
  it('handles thead content', function () {
    let { fragment } = htmx.makeFragment('<thead><tr><th>Header</th></tr></thead>')
  ...
  it('handles tbody content', function () {
    let { fragment } = htmx.makeFragment('<tbody><tr><td>Data</td></tr></tbody>')
  ...
  it('handles mixed table elements', function () {
    let { fragment } = htmx.makeFragment(
