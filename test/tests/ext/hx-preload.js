...
describe.skip('hx-preload attribute', function () {
  ...
  it('respects custom timeout', async function () {
    ...
    let btn = createProcessedHTML(
      '<button hx-get="/test" hx-preload="mouseenter timeout:100ms">Click</button>',
    )
    btn.dispatchEvent(new Event('mouseenter'))
    await htmx.timeout(20)
  ...
  it('builds URL with form params', async function () {
    ...
    let form = createProcessedHTML(
      '<form><input name="name" value="test"><button hx-get="/test" hx-preload="mouseenter">Click</button></form>',
    )
