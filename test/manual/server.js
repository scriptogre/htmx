...
const routes = {
  ...
  '/events': (req, res) => {
    ...
    res.write(
      `data: <htmx-action type="partial" hx-target="#events-output" hx-swap="beforeend"><div class="events-active"></div></htmx-action>\n\n`,
    )

    // Send initial status
    res.write(
      `data: <htmx-action type="partial" hx-target="#system-status" hx-swap="innerHTML">${status}</htmx-action>\n\n`,
    )
    ...
    const send = () => {
      ...
      res.write(
        `data: <htmx-action type="partial" hx-target="#activity" hx-swap="beforeend"><div>${activities[Math.floor(Math.random() * activities.length)]}</div></htmx-action>\n\n`,
      )
