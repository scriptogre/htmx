...
function cleanupTest() {
  ...
  if (typeof fetchMock !== 'undefined' && fetchMock.reset) {
    ...
    if (fetchMock.pendingRequests && fetchMock.pendingRequests.length > 0) {
      console.warn(
        `WARNING: Test is leaving ${fetchMock.pendingRequests.length} request(s) in flight. Tests should wait for all requests to complete.`,
      )
    }
    fetchMock.reset()
  }
...
function assertPropertyIs(css, property, content) {
  ...
  if (!elt) {
    assert.fail(
      "Could not find element with css '" + css + "' in :\n\n" + playground().innerHTML + '\n\n',
    )
  }
