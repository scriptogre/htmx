...
| `htmx_before_actions` | `htmx:before:actions` | Before actions run; `detail.actions` holds them; return `false` to skip |
...
| `api.runActions(actions, element, detail?)` | Run actions, firing `htmx:before:actions` and `htmx:after:actions` |
...
Pass custom event detail as the third argument:

```javascript
api.runActions(actions, element, {message});
```

Both action events receive `event.detail.message`.
...
```javascript
{
    ...
    actions: {          // Actions; HX-* response headers override attribute values
...
**Handling custom actions:** Unknown `HX-*` response headers become entries in `detail.ctx.actions` (`HX-Toast` → `toast`). Core ignores them; consume them in `htmx_before_actions` via `detail.actions`.
