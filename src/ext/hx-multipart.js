...
(() => {
    let HCON;
    let extractHxHeaders;

    htmx.registerExtension('multipart', {
        init: (api) => {
            HCON = api.HCON;
            extractHxHeaders = api.extractHxHeaders;
        },
        htmx_before_request: (element, detail) => {
            let request = detail.ctx.request;
            request.headers.append('Accept', 'multipart/mixed');
            request.headers.append('Accept', 'multipart/parallel');
        },

        htmx_before_response: (element, detail) => {
            let response = detail.ctx.response.raw;

            // Extract "multipart/*" from "multipart/*; boundary=updates".
            let [mediaType] = (response.headers.get('Content-Type') ?? '').split(';');
            mediaType = mediaType.trim().toLowerCase();

            // Skip if not multipart/* type.
            if (!['multipart/mixed', 'multipart/parallel'].includes(mediaType)) return;

            let ctx = detail.ctx;
            // Core already extracted envelope HX-* headers before this hook.
            // Parts own their HX-* headers, so prevent the envelope from acting as a part.
            ctx.hx = {};

            let originalText = response.text.bind(response);
            let handled = false;
            response.text = async () => {
                if (handled) return '';
                handled = true;
                ...
                for await (let part of response.parts()) {
                    ...
                    let handlePart = (async () => {
                        let partCtx = {
                            ...
                            selectOOB: defaults.selectOOB,
                            push: defaults.push,
                            replace: defaults.replace,
                            transition: defaults.transition,
                            hx: extractHxHeaders(part.headers),
                            text: await part.text(),
                            multipart: {type: mediaType, index: partIndex, part}
                        };

                        if (partCtx.hx.retarget) partCtx.target = partCtx.hx.retarget;   // HX-Retarget
                        if (partCtx.hx.reswap) partCtx.swap = partCtx.hx.reswap;       // HX-Reswap
                        if (partCtx.hx.reselect) partCtx.select = partCtx.hx.reselect; // HX-Reselect

                        if (handleHeadersAndMaybeReturnEarly(partCtx)) {
                            ctx.keepIndicators = true;
                            return false;
                        }

                        await htmx.swap(partCtx);
                        return true;
                    })();

                    if (mediaType === 'multipart/parallel') {
            ...
            response.text.original = originalText;
        }
    });

    // BEGIN copied from src/htmx.js response header handling.
    // Keep this block mechanically close to src/htmx.js.
    function handleTriggerHeader(value, elt) {
        if (value[0] === '{') {
            ...
            for (let name in triggers) {
                ...
                let target = elt;
                if (detail?.target) {
                    target = htmx.find(detail.target);
                }
                htmx.trigger(target, name, typeof detail === 'object' ? detail : {value: detail});
            }
        } else {
            value.split(',').forEach(name => htmx.trigger(elt, name.trim(), {}));
        }
    }

    function handleHeadersAndMaybeReturnEarly(ctx) {
        if (ctx.hx.trigger) { // HX-Trigger
            handleTriggerHeader(ctx.hx.trigger, ctx.sourceElement);
        }
        ...
        if (ctx.hx.location) { // HX-Location
            ...
            opts.push ??= 'true';
            htmx.ajax('GET', path, opts);
            return true;
        }
    }
    // END copied from src/htmx.js response header handling.
