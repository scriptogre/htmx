...
async fn api_extensions(State(state): State<AppState>) -> impl IntoResponse {
    match read_register_entries(&state.root) {
        Ok(entries) => {
...
fn render_page_response(state: &AppState) -> Result<String, String> {
    let entries = read_register_entries(&state.root)?;
...
fn assemble_selected(root: &Path, selected: &HashSet<String>, inline: bool) -> Result<AssembleResult, String> {
    ...
    let register_entries = extract_register_entries(&core);
    let mut filtered_lines: Vec<String> = core
        .lines()
        .filter(|line| !line.contains("htmx.register("))
    ...
    for entry in &register_entries {
        if selected.contains(&entry.name) {
            filtered_lines.push(format!("htmx.register('{}', {})", entry.name, entry.arg_expr));
        }
    }
...
fn extract_register_entries(core_source: &str) -> Vec<RegisterEntry> {
    core_source
        .lines()
        .filter_map(parse_register_line)
        .collect()
}

fn parse_register_line(line: &str) -> Option<RegisterEntry> {
    let trimmed = line.trim_start();
    if trimmed.starts_with("//") {
        return None;
    }
    let rest = trimmed.strip_prefix("htmx.register(")?;
    ...
    Some(RegisterEntry { name, arg_expr })
}
