...
Here we find ourselves in violent agreement with Mr. Harris, with the caveat that we would substitute "the _javascript_ web platform"
...
Now, a vanilla MPA would typically have said garbage cached after the first request, so the download cost, at least, is about the same as with SPAs. But an MPA must _execute_ the bundle of garbage again on each page, which does burn CPU and can lead to poor user experience.
...
It turns out, however, that those usability issues often _can_ [be addressed](@/patterns/_index.md) using The Hypermedia Approach:
rather than _abandoning_ Hypermedia for RPC, what we needed then and what we need today is a _more powerful_ Hypermedia.
...
But this _does_ imply that the future of the web does not _necessarily_ belong _entirely_ to javascript, as appeared to be the case
say five years ago.
