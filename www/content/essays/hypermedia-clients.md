...
And so on: the sorts of pedantic nit-picking that makes technical flame wars about REST on the internet such a _special_ joy.

However, there is a deeper [ackchyually](https://i.imgur.com/DpQ9YJl.png) here, and one that doesn't involve the _JSON API_
itself, but rather the other side of the wire: the _client_ that receives the JSON.
...
client isn't trivial, and, further, it is certainly not what _most_ engineers would build to consume a JSON API, even if
...
but not the entire story. And, I have come to understand, it is not really the _hard_ part of the story. In fact, creating the
hypermedia _client_ is the hard part, and creating a _good_ hypermedia client is _the really hard part_.
...
parsing and rendering HTML to an end user in a normal, every day web request. It's _extremely_ complicated.
...
than a compelling, practical and _complete_ RESTful hypermedia solution.
...
It took me a long time to appreciate how important the _client_ is to a proper, RESTful hypermedia system. This is understandable,
...
your hypermedia API really work, _and_ you are going to need a properly built hypermedia client to consume it.
