...
```html
<button hx-post="/submit" hx-status:422="select:#errors" hx-status:500="select:#server-error">
  Submit
</button>
```
...
```html
<form hx-post="/register" hx-status:422="select:#errors target:#error-container">
...
```html
<div hx-get="/user/123" hx-status:404="select:#user-not-found">Loading...</div>
```
...
```html
<button hx-get="/data" hx-push-url="true" hx-status:4xx="push:false" hx-status:5xx="push:false">
  Load Data
</button>
```
...
```html
<form hx-post="/items" hx-status:201="push:/items/new">
  <!-- form fields -->
</form>
```
