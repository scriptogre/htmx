  ...
  class Htmx {
    ...
    constructor() {
      ...
      this.#hxOnQuery = new XPathEvaluator().createExpression(
        `.//*[@*[ starts-with(name(), "${this.#prefix('hx-on')}")]]`,
      )
      this.#internalAPI = {
    ...
    #attributeValue(elt, name, defaultVal, returnElt) {
      ...
      let inheritName =
        name + (this.config.implicitInheritance ? '' : this.#maybeAdjustMetaCharacter(':inherited'))
    ...
    #determineMethodAndAction(elt, evt) {
      if (this.#isBoosted(elt)) {
        ...
        return { shouldActivate: action, method }
      }
    }

    #boostedMethodAndAction(elt, evt) {
      if (elt.matches('a')) {
        return { shouldActivate: elt.getAttribute('href'), method: 'GET' }
      } else {
    ...
    #createRequestContext(sourceElement, sourceEvent) {
      ...
      let ctx = {
        ...
        target: this.#resolveTarget(
          sourceElement,
          this.#attributeValue(sourceElement, 'hx-target'),
        ),
    ...
    async #issueRequest(ctx) {
      ...
      try {
        ...
        let response = await ctx.fetch(ctx.request.shouldActivate, ctx.request)
    ...
    async #handleSSE(ctx, elt, response) {
      ...
      while (elt.isConnected) {
        ...
        if (attempt > 0) {
          ...
          let delay = Math.min(
            this.parseInterval(config.initialDelay) * Math.pow(2, attempt - 1),
            this.parseInterval(config.maxDelay),
          )
          let reconnect = { attempt, delay, lastEventId, cancelled: false }

          ctx.status = 'reconnecting to stream'
          if (
            !this.#trigger(elt, 'htmx:before:sse:reconnect', {
              ctx,
              reconnect,
            }) ||
            reconnect.cancelled
          ...
          try {
            ...
            currentResponse = await fetch(ctx.request.shouldActivate, ctx.request)
          } catch (e) {
        ...
        try {
          for await (const sseMessage of this.#parseSSE(currentResponse)) {
            ...
            if (
              !this.#trigger(elt, 'htmx:before:sse:message', {
                ctx,
                message: msg,
              }) ||
              msg.cancelled
    ...
    #shouldBoost(elt) {
      if (this.#shouldInitialize(elt)) {
        if (elt.tagName === 'A') {
          ...
          return elt.method !== 'dialog' && this.#isSameOrigin(elt.shouldActivate)
        }
      }
    }
      ...
      for (let templateElt of fragment.querySelectorAll('template[hx]')) {
        ...
        if (type === 'partial') {
          let swapSpec = this.#parseSwapSpec(
            templateElt.getAttribute(this.#prefix('hx-swap')) || this.config.defaultSwap,
          )
    ...
    ajax(verb, path, context) {
      ...
      Object.assign(ctx.request, { shouldActivate: path, method: verb.toUpperCase() })
    ...
    #addInputValues(elt, included, formData) {
      let inputs = this.#queryEltAndDescendants(
        elt,
