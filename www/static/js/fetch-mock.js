...
window.fetch = async function (url, init = {}) {
  ...
  const route = routes.find(
    r =>
      r.method === method &&
      (r.url instanceof RegExp ? r.url.test(urlWithoutQuery) : r.url === urlWithoutQuery),
  )
  ...
  const body = route.handler(
    { url, method, body: init.body, headers: init.headers },
    parseParams(method, url, init.body),
    headers,
  )
...
function parseParams(method, url, body) {
  const parse = str => {
    ...
    str?.replace(
      /([^&=]+)=([^&]*)/g,
      (_, k, v) => (params[decodeURIComponent(k)] = decodeURIComponent(v.replace(/\+/g, ' '))),
    )
...
function params(request) {
  return parseParams(request.method, request.url, request.body)
}
function headers(request) {
  const hx = {}
  for (const [k, v] of Object.entries(request.headers || {}))
    if (k.toLowerCase().startsWith('hx-')) hx[k] = v
  return hx
}
