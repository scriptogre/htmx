describe('Server-Sent Events', function () {
  ...
  it('continuous stream reconnects with exponential backoff', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/reconnect" hx-config="sse.continuous:true sse.initialDelay:50ms sse.maxRetries:3" hx-swap="innerHTML">Connect</button>',
    )
  ...
  it('Last-Event-ID header sent on reconnect', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/with-id" hx-config="sse.continuous:true sse.initialDelay:50ms sse.maxRetries:2" hx-swap="innerHTML">Connect</button>',
    )
  ...
  it('element removal stops streaming', async function () {
    ...
    createProcessedHTML(
      '<div id="container"><button hx-get="/removable" hx-config="sse.continuous:true sse.initialDelay:50ms" hx-swap="innerHTML">Connect</button></div>',
    )
  ...
  it('maxRetries configuration works', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/max-retries" hx-config="sse.continuous:true sse.initialDelay:20ms sse.maxRetries:2" hx-swap="innerHTML">Connect</button>',
    )
  ...
  it('maxDelay configuration caps backoff', async function () {
    ...
    createProcessedHTML(
      '<button hx-get="/max-delay" hx-config="sse.continuous:true sse.initialDelay:20ms sse.maxDelay:60ms" hx-swap="innerHTML">Connect</button>',
    )
