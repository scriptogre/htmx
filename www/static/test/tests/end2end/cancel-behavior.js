describe('Cancel behavior integration tests', function () {
  ...
  it('htmx-enabled button with sub-elements prevents form submission', async function () {
    ...
    const form = createProcessedHTML(
      '<form><button id="btn" hx-get="/foo"><span id="span">test</span></button></form>',
    )
  ...
  it('htmx-enabled element inside form button prevents form submission', async function () {
    ...
    mockResponse('GET', '/foo', 'Response')
    const form = createProcessedHTML(
      '<form><button id="btn"><span id="span" hx-get="/foo">test</span></button></form>',
    )
  ...
  it('from: trigger on form prevents default form submission', async function () {
    ...
    createProcessedHTML(
      '<form id="test-form" action="/submit"><input type="submit" id="submit" value="Submit"></form><div hx-post="/test" hx-trigger="submit from:#test-form"></div>',
    )
  ...
  it('from: trigger on button prevents default form submission', async function () {
    ...
    mockResponse('POST', '/test', 'Response')
    createProcessedHTML(
      '<form><button id="test-btn" type="submit">Submit</button></form><div hx-post="/test" hx-trigger="click from:#test-btn"></div>',
    )
  ...
  it('from: trigger on link prevents default navigation', async function () {
    ...
    createProcessedHTML(
      '<a id="test-link" href="#">Go to page</a><div hx-get="/test" hx-trigger="click from:#test-link"></div>',
    )
  ...
  it('modified click trigger on form does not prevent default on other elements', async function () {
    ...
    createProcessedHTML(
      '<input type="date" id="datefield"><form hx-trigger="click from:body"></form>',
    )
  ...
  it('boosted anchor prevents default navigation', async function () {
    ...
    createProcessedHTML(
      '<div hx-boost:inherited="true" hx-target:inherited="this"><a id="test-link" href="#test">Go</a></div>',
    )
  ...
  it('boosted form prevents default submission', async function () {
    ...
    createProcessedHTML(
      '<div hx-boost:inherited="true" hx-target:inherited="this"><form id="test-form" action="/test" method="post"><button id="btn">Submit</button></form></div>',
    )
  ...
  it('does not submit with false condition on form', async function () {
    ...
    mockResponse('POST', '/test', 'Submitted')
    createProcessedHTML(
      '<form hx-post="/test" hx-trigger="submit[false]"><button id="btn">submit</button></form>',
    )
