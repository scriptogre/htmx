...
htmx_before_swap: (elt, detail) => {
    if (detail.ctx.response.status !== 200) {
        var target = getRespCodeTarget(detail.ctx.sourceElement, detail.ctx.response.status);
        if (target) {
...
Event flow: response received, `ctx.swap.content` set, `htmx:after:request` fires, content is resolved into `ctx.swaps`, `htmx:before:swaps` fires once for the batch, then `htmx:before:swap` / `htmx:after:swap` fire around each individual swap.
...
| Hook Name | Triggered Event | Parameters | Description |
|-----------|----------------|------------|-------------|
| `htmx_before_swaps` | [`htmx:before:swaps`](/reference/events/htmx-before-swaps) | `(elt, detail)` | Before resolved swaps |
| `htmx_after_swaps` | [`htmx:after:swaps`](/reference/events/htmx-after-swaps) | `(elt, detail)` | After resolved swaps |
| `htmx_before_swap` | [`htmx:before:swap`](/reference/events/htmx-before-swap) | `(elt, detail)` | Before each resolved swap |
| `htmx_after_swap` | [`htmx:after:swap`](/reference/events/htmx-after-swap) | `(elt, detail)` | After each resolved swap |
| `htmx_finally_swap` | [`htmx:finally:swap`](/reference/events/htmx-finally-swap) | `(elt, detail)` | Always called after swap handling |
