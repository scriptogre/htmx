...
| [`target`](/reference/attributes/hx-swap#target) | `Element` | Element where content will be swapped. |
...
Fields may be absent when they do not apply or before their lifecycle value is available. Swap behavior fields correspond to the [`hx-swap`](/reference/attributes/hx-swap) modifiers.
