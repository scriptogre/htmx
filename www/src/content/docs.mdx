...
| [`htmx:done`](/reference/events/htmx-done)                                   | When the request → response → swap pipeline ends  |
...
| `htmx_done` | [`htmx:done`](/reference/events/htmx-done) | `(elt, detail)` | When the pipeline ends |
...
- `resolveTarget(elt, target)` - Resolve a target selector or element
