...
| [`htmx:before:actions`](/reference/events/htmx-before-actions)               | Before actions run (cancellable)                  |
| [`htmx:after:actions`](/reference/events/htmx-after-actions)                 | After actions run                                 |
...
See the [request lifecycle](/reference/events).
...
htmx decodes `HX-*` response headers into an actions object on the request context: `HX-Push-Url` becomes `ctx.actions.pushUrl`, and so on. `HX-Retarget`, `HX-Reswap`, and `HX-Reselect` are swap directives and update `ctx.swap` instead. Core runs all actions once before the swap. See [sources](/reference/events/htmx-before-actions#when-it-fires) and [formats](/reference/events/htmx-before-actions#actions).
...
- `runActions(actions, element, detail?)` - Run actions, firing `htmx:before:actions` and `htmx:after:actions`
...
```javascript
{
    ...
    actions: {         // Actions from attributes and HX-* response headers
