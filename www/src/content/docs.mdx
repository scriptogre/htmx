...
Core events follow `htmx:<timing>:<noun>[:<type>]`. Most error events are consolidated to [`htmx:error`](/reference/events/htmx-error). HTTP error responses use [`htmx:response:error`](/reference/events/htmx-response-error).
...
| `htmx:afterRequest`         | [`htmx:after:response`](/reference/events/htmx-after-response)                    | renamed | Response body is available.          |
...
| —                 | `Accept`                                                | new             | Core requests send `text/html`; [`hx-sse`](/extensions/hx-sse#accept) adds `text/event-stream`. |
...
| [`htmx:after:request`](/reference/events/htmx-after-request)                 | Immediately after fetch resolves                  |
...
| [`htmx:before:actions`](/reference/events/htmx-before-actions)               | Before server actions run (cancellable)           |
| [`htmx:after:actions`](/reference/events/htmx-after-actions)                 | After server actions run                          |
| [`htmx:done`](/reference/events/htmx-done)                                   | When an issuing pipeline completes, fails, or is cancelled |
...
| [`hx-head`](/extensions/hx-head)                | Merges head tag information (styles, etc.) in htmx requests                          |
...
| [`sse`](/extensions/hx-sse)                             | Server-Sent Events streaming support ([upgrade guide](/extensions/hx-sse#migration)) |
| [`upsert`](/extensions/hx-upsert)                       | Updates existing elements by ID and inserts new ones, preserving unmatched elements  |
| [`ws`](/extensions/hx-ws)                               | Bi-directional WebSocket communication ([upgrade guide](/extensions/hx-ws#migration)) |
...
- `detail.ctx.swap.target` (selector or explicit element input)
...
htmx_before_swap: (elt, detail) => {
    if (detail.ctx.response.status !== 200) {
        ...
        var mainTask = detail.tasks.find(task => task.type === 'main');
        if (target && mainTask) {
...
Event flow: response received, `htmx:after:request` fires, the body is read into `ctx.swap.content`, `htmx:after:response` fires, then htmx builds the fragment before `htmx:before:swap`.
...
See the [event lifecycle](/reference/events) for the complete request → response → swap order.
...
htmx decodes `HX-*` response headers into an actions object on the request context: `HX-Push-Url` becomes `ctx.actions.pushUrl`, and so on. `HX-Retarget`, `HX-Reswap`, and `HX-Reselect` are swap directives and update `ctx.swap` instead. Each set of actions runs between [`htmx:before:actions`](/reference/events/htmx-before-actions) and [`htmx:after:actions`](/reference/events/htmx-after-actions).

Unknown `HX-*` headers become custom actions: `HX-Toast: Saved!` arrives as `ctx.actions.toast`. Core ignores custom actions; handle them in the action events:
...
| `htmx_before_response` | [`htmx:before:response`](/reference/events/htmx-before-response) | `(elt, detail)` | Before body consumption |
| `htmx_after_response` | [`htmx:after:response`](/reference/events/htmx-after-response) | `(elt, detail)` | After body consumption |
| `htmx_response_error` | [`htmx:response:error`](/reference/events/htmx-response-error) | `(elt, detail)` | On HTTP status 400 or higher |
| `htmx_before_actions` | [`htmx:before:actions`](/reference/events/htmx-before-actions) | `(elt, detail)` | Before server actions run |
| `htmx_after_actions` | [`htmx:after:actions`](/reference/events/htmx-after-actions) | `(elt, detail)` | After server actions run |
| `htmx_error` | [`htmx:error`](/reference/events/htmx-error) | `(elt, detail)` | On an exception |
| `htmx_done` | [`htmx:done`](/reference/events/htmx-done) | `(elt, detail)` | When an issuing pipeline completes, fails, or is cancelled |
...
- `createRequestContext(elt, evt, overrides)` - Create a request context
- `resolveTarget(elt, target)` - Resolve a target from its source element
...
- `runActions(actions, element)` - Run server actions, firing `htmx:before:actions` and `htmx:after:actions`
...
The `detail.ctx` object contains request and response information:
...
```javascript
{
    ...
    actions: {         // Server actions from attributes and HX-* response headers
...
`ctx.swap` describes the content swap. Structured swaps use the same fields.

| Field | Type | Description |
|---|---|---|
| `content` | `string` | HTML content to swap. |
| [`target`](/reference/attributes/hx-swap#target) | `string \| Element \| undefined` | Target selector or explicit element. Relative selectors resolve from `sourceElement`. |
| [`style`](/reference/attributes/hx-swap#swap-methods) | `string` | Swap style, such as `innerHTML` or `outerHTML`. |
| [`select`](/reference/attributes/hx-select) | `string` | CSS selector used to extract content from the response. |
| [`selectOOB`](/reference/attributes/hx-select-oob) | `string` | Selector for out-of-band swaps. |
| [`transition`](/reference/attributes/hx-swap#transition) | `boolean` | Whether to use the View Transitions API. |
| [`swapDelay`](/reference/attributes/hx-swap#swap) | `string \| number` | Delay before the swap. |
| [`settleDelay`](/reference/attributes/hx-swap#settle) | `string \| number` | Delay before settling. |
| [`scroll`](/reference/attributes/hx-swap#scroll) | `"top" \| "bottom"` | Scroll position after the swap. |
| [`scrollTarget`](/reference/attributes/hx-swap#scroll) | `string` | Target for scrolling. |
| [`show`](/reference/attributes/hx-swap#show) | `"top" \| "bottom" \| "none"` | Position at which to show an element. |
| [`showTarget`](/reference/attributes/hx-swap#show) | `string` | Target to show. |
| [`ignoreTitle`](/reference/attributes/hx-swap#ignoretitle) | `boolean` | Whether to ignore a response title. |
| [`focusScroll`](/reference/attributes/hx-swap#focusscroll) | `boolean` | Whether to scroll a focused element into view. |
| [`swapEmpty`](/reference/attributes/hx-swap#swapempty) | `boolean` | Whether empty content performs the main swap. |
| [`strip`](/reference/attributes/hx-swap#strip) | `boolean` | Whether to strip the response wrapper. |

Fields may be absent when they do not apply or before their lifecycle value is available. `target` preserves its selector or explicit element input and remains `undefined` when omitted. Swap tasks resolve selectors when they need an element. Swap behavior fields correspond to the [`hx-swap`](/reference/attributes/hx-swap) modifiers.
