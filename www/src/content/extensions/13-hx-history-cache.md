...
2. **On back/forward** (`htmx:before:history:restore`): the extension looks up the target path in the cache.
   - **Hit**: fires `htmx:history:cache:hit`, cancels the core network path, and restores content via `htmx.swap()`. If `hx-head` is loaded, it coordinates head restoration through the cache before and after restore events.
