---
title: "Hotwire / Turbo → htmx Migration Guide"
description: "Common practices for migrating from Hotwire/Turbo to htmx"
---
...
* `<body hx-boost="true">` to enable a Turbo Drive-like experience. See: [hx-boost](/reference/attributes/hx-boost)
...
* Or, [hx-on](/reference/attributes/hx-on) may be used:
...
See: [Event Naming](/docs#event-naming)
...
* Or, use an [hx-trigger](/reference/attributes/hx-trigger) condition: `hx-trigger="submit[action(target)]"`
  * Does not currently resolve async calls. See [issue](https://github.com/bigskysoftware/htmx/issues/912)
* Or, use [hx-on](/reference/attributes/hx-on): `hx-on:click="event.preventDefault(); action(this); htmx.trigger(this, 'ready')"` `hx-trigger="ready"`
...
* [hx-on](/reference/attributes/hx-on) provides an inline, vanilla substitute for a wide variety of use cases.
