...
| [`hx-on*`](/attributes/hx-on)            | handle events with inline scripts on elements                                                 |
| [`hx-push-url`](/attributes/hx-push-url) | push a URL into the browser location bar to create history                                    |
| [`hx-select`](/attributes/hx-select)     | select content to swap in from a response                                                     |
| [`hx-swap`](/attributes/hx-swap)         | controls how content will swap in (`outerHTML`, `innerHTML`, `beforeend`, `afterend`, ...)    |
| [`hx-swap-oob`](/attributes/hx-swap-oob) | mark element to swap in from a response (out of band)                                         |
...
| Attribute                                          | Description                                                                                                  |
|----------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| [`hx-action`](/attributes/hx-action)     | specifies the URL to issue the request to                                                     |
| [`hx-boost`](/attributes/hx-boost)             | add [progressive enhancement](https://en.wikipedia.org/wiki/Progressive_enhancement) for links and forms     |
| [`hx-config`](/attributes/hx-config)           | configure request behavior with JSON                                                                         |
| [`hx-confirm`](/attributes/hx-confirm)         | shows a `confirm()` dialog or runs async confirmation before issuing a request                               |
| [`hx-delete`](/attributes/hx-delete)           | issues a `DELETE` to the specified URL                                                                       |
| [`hx-disable`](/attributes/hx-disable)         | adds the `disabled` attribute to the specified elements while a request is in flight                         |
...
| [`hx-ignore`](/attributes/hx-ignore)           | disables htmx processing for the given node and any children nodes                                           |
...
| [`hx-method`](/attributes/hx-method)     | specifies the HTTP method to use                                                              |
| [`hx-optimistic`](/attributes/hx-optimistic)   | show optimistic content while request is in flight                                                           |
| [`hx-patch`](/attributes/hx-patch)             | issues a `PATCH` to the specified URL                                                                        |
| [`hx-preload`](/attributes/hx-preload)         | preload a request on a trigger event                                                                         |
