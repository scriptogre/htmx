...
and we will demonstrate how an MPA using a hypermedia-oriented technology, [htmx](/_index), can solve each of them.
...
That being said, htmx supports using [standard CSS transitions](/patterns/animations) to make animations possible.  Obviously there is a limit to what you can achieve with these pure CSS techniques, but we believe this can give you the 80 of an 80/20 situation.  (Or, perhaps, the 95 of a 95/5 situation.)
...
htmx offers multiple ways to [update content beyond the target element](/patterns/update-other-content), all of them quite easy and any of which would work to solve the UI consistency issues Mr. Harris points out.
...
It turns out, however, that those usability issues often *can* [be addressed](/patterns/_index) using The Hypermedia Approach:
