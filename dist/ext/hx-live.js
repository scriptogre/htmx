htmxLive = (() => {
  class HtmxLive {
    ...
    #onMutation(records) {
      ...
      let toRefresh = []
      outer: for (const dependency of this.#dependencies) {
        for (let record of records) {
  ...
  class Tokenizer {
    ...
    validCss(c) {
      return (
        this.match(this.ALPHA, c) ||
        this.match(this.NUMERIC, c) ||
        c === '-' ||
        c === '_' ||
        c === ':'
      )
  ...
  class Parser {
    ...
    match(term, type) {
      return (
        (this.currentToken().val === term || term === null) &&
    ...
    require(term, type = 'sym') {
      ...
      if (!match)
        throw 'Expected: ' + (term ? term : type) + ' found ' + JSON.stringify(this.currentToken())
      return match
    }
    ...
    endOfCommands() {
      return this.matchAndConsume('end', 'sym') || this.match('on') || this.match('bind')
    }
